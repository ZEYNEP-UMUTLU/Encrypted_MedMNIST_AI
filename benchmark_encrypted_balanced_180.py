...
# ---------------------------------------------------------
# BALANCED SAMPLE SELECTION: 20 samples per class
# ---------------------------------------------------------

labels = np.asarray(test_ds.labels).reshape(-1)

class_indices = {
    c: np.where(labels == c)[0][:N_PER_CLASS].tolist()
    for c in range(9)
}
...
assert len(balanced_indices) == N_SAMPLES

print('Balanced class distribution:')
for c in range(9):
    print(f'  Class {c}: {len(class_indices[c])} samples')

# ---------------------------------------------------------
# BENCHMARK
# ---------------------------------------------------------

for i, dataset_idx in enumerate(balanced_indices):

    image, label = test_ds[dataset_idx]
...
with open("encrypted_benchmark_balanced_180.csv", "w", newline="") as f:
...
print()
print("Saved: encrypted_benchmark_balanced_180.csv")
